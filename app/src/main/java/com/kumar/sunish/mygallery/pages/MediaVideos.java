 ...
 */
public class MediaVideos implements DynamicPage {
    ...
    public NanoHTTPD.Response createResponse(MyHTTPD server, NanoHTTPD.IHTTPSession session) {
        ...
        try {
            ...
            ArrayList<Map<String, String>> list = util.getListForGroup(MediaUtils.VIDEO, id);

            for (Map<String, String> image : list) {


                if (isApp) {
                    msg += "<div class=\"item \"><a href=\"javascript:viewVideo('" + image.get("ID") + "');\"><img class=\"lazy\" src=\"/blank_img.png\" data-original=\"/pages/Thumb?type=" + MediaUtils.VIDEO + "&id=" + image.get("ID") + "\" ></a>\n";
                } else {
                    msg += "<div class=\"item \"><a href=\"/pages/Gallery?type=" + MediaUtils.VIDEO + "&id=" + image.get("ID") + "\" target=\"_newview\"><img class=\"lazy\" src=\"/blank_img.png\" data-original=\"/pages/Thumb?type=" + MediaUtils.VIDEO + "&id=" + image.get("ID") + "\" ></a>\n";
                }
                msg += "      " + image.get("DATE") + "\n" +
                        "      " + image.get("SIZE") + "\n";
                if (isApp) {
            ...
            msg += "</div></div>";
            if (isApp) {
                msg += "<a href=\"javascript:download('" + MediaUtils.VIDEO + "');\"><img src=\"/arrow-download-icon.png\" id=\"fixedbutton\"></a>";
            }
